...
import emoji
import time
# BUG: Due to Youtube API having been changed...so this code is currently not working now lol..........
...
class ChatReplayDownloader:
    ...
    def __get_initial_youtube_info(self, video_id):
        ...
        json_data = next(line.strip()[len('var ytInitialData = '):-1]  # I think the API of YTB changed or sth. Old one: window["ytInitialData"] = 
            ...
            if(args.output.endswith('.json')):
                ...
                for message in chat_messages:
                    if('ticker_duration' not in message):  # needed for duplicates
                        num_of_messages += 1
                        #json.dump(chat_downloader.message_to_string(message), f)
